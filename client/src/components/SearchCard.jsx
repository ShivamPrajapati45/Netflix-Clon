...
const SearchCard = React.memo(({movieData, isLiked=false}) => {
    ...
    >
        {movieData?.backdrop_path ? (
            <img src={`https://image.tmdb.org/t/p/w500${ movieData.image || movieData?.backdrop_path || movieData?.poster_path }`} alt="movie1"/>
        ) : <img src={`https://image.tmdb.org/t/p/w500${ movieData.image || movieData?.poster_path || movieData?.backdrop_path }`} className='image' alt='movie2'/>
        }
                ...
                <div className="image-video-container">
                    {
                        ...
                        (
                            <ReactPlayer url={`https://www.youtube.com/watch?v=${trailerFromStore?.length > 0 ? trailerFromStore[0]?.key : ""}`} playing={true} className="react-player"  width='100%' height='100%' controls={true}/>
                        ) 
                            : 
                        (
                    ...
                    <div className="icons flex  j-between">
                        <div className="controls flex">
                            ...
                            <RiThumbUpFill title='like'
                                        ...
                                        className={like === true ? "like" : "default"} />
                            <RiThumbDownFill title='dislike' 
                                                onClick={() => setLike(false)} 
                                                className={like === false ? "dislike" : "default"} />
                            {
                                isLiked ? 
...
const Container = styled.div`
    ...
    .dislike {
        color: red; /* Changed to red for dislike */
        transform: scale(1.2);
        transition: transform 0.3s ease-in-out, color 0.3s ease-in-out;
    }
    .image{
